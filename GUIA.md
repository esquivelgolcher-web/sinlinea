...
- **Fuentes de tendencias**: además de RSS y portada, una fuente puede ser de tipo
  **Tendencias** y apuntar a `https://trends.google.com/trending/rss?geo=XX`, con el código
...
**Carrusel:** muestra sus diapositivas ya renderizadas y se aprueba como un post (hora, destinos y texto por red); al aprobar quedan registradas todas las diapositivas en su orden. Sale como carrusel en Instagram (hasta 10 diapositivas) y en Threads (hasta 20; el sistema renderiza como máximo 10). Facebook no aparece como destino de carruseles porque esa publicación aún no se ha validado con la API real: el diálogo lo dice, y Facebook sigue publicando imágenes individuales. Si el carrusel aún no tiene todas sus diapositivas renderizadas, el panel no deja aprobarlo hasta que existan. **Reel:** muestra la narración, los subtítulos, las escenas y los recursos; no se puede programar todavía (al pulsar Aprobar el panel lo dice) y tu cuenta ya no genera reels.
