...
**Carruseles publicables en Instagram y Threads (desplegado el 2026-09-10; probado solo con clientes simulados, sin publicaciones reales):** adaptadores en `lib/instagram.mjs` y `lib/threads.mjs` (contenedor hijo por diapositiva, `CAROUSEL` con `children` en orden); límites por red respaldados por la documentación oficial (`CARRUSEL_POR_RED`: Instagram hasta 10 y solo JPEG; Threads 2-20) validados antes de aprobar y en el publicador; publicador con los hijos subidos al remoto antes del padre y el padre antes de publicar, reconciliación con hijos y guarda para carruseles sin diapositivas; aprobación de todas las imágenes en su orden (`aprobado.imagenesSha`, `/api/archivo-sha`, `huellaArchivo` en local y GitHub) y «Aprobar imágenes actuales» en el panel; además, un intento que una corrida interrumpida dejó en fase `enviando` pasa a incierto (todas las redes y formatos) y un destino en error recuerda su último intento para reutilizar contenedores al reintentar. **Facebook queda fuera de los carruseles** por decisión del operador hasta validar con la API real la publicación con varias fotos (`lib/facebook.mjs` conserva `attached_media` múltiple y la evidencia por `subattachments`, sin usar; el panel y el publicador lo explican); sigue publicando imágenes individuales. Reels desactivados en la generación de la cuenta personal (`perfil.formatos: ["post", "carrusel"]`). **Validación real:** primer carrusel publicado en Instagram el 2026-09-10 a las 18:08 UTC (Clearview, WIRED 3cbb, seis diapositivas; corrida PUBLICAR 34512410209; reserva → hijos → padre → enviando → publicado en cinco commits del bot; https://www.instagram.com/p/DdHcfgLIFzl/), aprobado expresamente por el operador solo para Instagram. Primer carrusel publicado en Threads el mismo día a las 18:48 UTC (la misma pieza, Threads añadido como destino nuevo sin tocar Instagram; corrida PUBLICAR 34516524906; cinco commits del bot; https://www.threads.com/@luisegolcher/post/DdHhIhpiCZW), con la propuesta automática aprobada por el operador. **Carruseles cerrados en Instagram y Threads.** Solo simulado: la recuperación tras cortes y los inciertos (ninguno real hasta ahora); si un envío queda incierto no se repite sin evidencia. Facebook sigue fuera de los carruseles y los reels fuera del alcance actual.

**Pendiente (integraciones):** adaptador de reels (producción de vídeo y voz, que el proyecto no tiene; los reels están desactivados en la cuenta personal); validación real de carruseles; edición de diapositivas y guion desde el panel; métricas de las piezas nuevas. Sin credenciales adicionales: todo usa las claves de Claude y Gemini existentes.
