...
**Carruseles publicables (2026-09-10, rama `carrusel-publicacion`, sin push ni publicaciones reales):** adaptadores en `lib/instagram.mjs` y `lib/threads.mjs` (contenedor hijo por diapositiva, `CAROUSEL` con `children` en orden) y en `lib/facebook.mjs` (fotos sin publicar y `attached_media` múltiple; evidencia por `attachments` o `subattachments`); publicador con los hijos subidos al remoto antes del padre y el padre antes de publicar, reconciliación con hijos y guarda para carruseles sin diapositivas; aprobación de todas las imágenes en su orden (`aprobado.imagenesSha`, `/api/archivo-sha`, `huellaArchivo` en local y GitHub) y «Aprobar imágenes actuales» en el panel; además, un intento que una corrida interrumpida dejó en fase `enviando` pasa a incierto (todas las redes y formatos) y un destino en error recuerda su último intento para reutilizar contenedores al reintentar. Reels desactivados en la generación de la cuenta personal (`perfil.formatos: ["post", "carrusel"]`, en main desde el 2026-09-10). Probado solo con clientes simulados (pruebas unitarias de clientes, modelo y publicador; e2e del panel); pendiente: validar con la API real pieza a pieza con aprobación expresa del operador. La publicación con varias fotos en Facebook y la evidencia por `subattachments` no están confirmadas en la documentación oficial.
