...
- Carruseles (formato `carrusel`, entre 2 y 10 diapositivas renderizadas): Instagram y Threads crean un contenedor hijo por diapositiva y un contenedor `CAROUSEL` con `children` en el orden aprobado; Facebook sube una foto sin publicar por diapositiva y publica una sola entrada con `attached_media`. Los ids de los hijos (`intento.hijos`) se suben al remoto antes de crear el padre y el padre antes de publicar; un corte reutiliza lo creado y nunca duplica. Un carrusel sin sus diapositivas renderizadas espera; el reel sigue sin enviarse.
...
- **Formatos.** `post` se publica como siempre (imagen + texto por red). `carrusel` renderiza una imagen por diapositiva (`templates/carrusel.html` → `public/img/<id>-01.jpg`…) y `reel` guarda el guion. **El carrusel se publica** (entre 2 y 10 diapositivas renderizadas; ver §12.3): Instagram y Threads con contenedores hijos y `CAROUSEL`, Facebook con varias fotos en una publicación; al aprobar, el panel guarda la huella de cada diapositiva en su orden y pide «Aprobar imágenes actuales» si cambian. **El reel no tiene adaptador de publicación**: el panel lo muestra para revisión y no deja programarlo; el publicador tampoco lo envía (haría falta un flujo de vídeo que hoy no existe). En la cuenta personal `perfil.formatos` es `["post", "carrusel"]` por decisión del operador (2026-09-10): no se generan reels.
...
**Estado (2026-09-10):** implementado y probado con clientes simulados (agrupación y duplicados, artículo inaccesible sin resumen inventado, hecho antiguo con su fecha, denuncias atribuidas, render del carrusel real con Chromium, borradores sin publicar, fuentes conservadas hasta el post). Primeros borradores reales generados con la corrida de GENERAR indicada en el ROADMAP. La publicación de carruseles se implementó el 2026-09-10 en la rama `carrusel-publicacion` y está probada solo con clientes simulados (sin publicaciones reales): flujo completo en las tres redes con persistencia intermedia, huellas por diapositiva y recuperación sin duplicados. Pendiente de validar con la API real pieza a pieza y con aprobación expresa; la publicación con varias fotos en Facebook y la evidencia por `subattachments` no se confirmaron en la documentación oficial.
